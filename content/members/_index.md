---
title: "Members"
---
<style>
@media (max-width: 600px) {
  .member-block {
    flex-direction: column !important;
    align-items: center !important;
    text-align: center;
  }
  .member-block img {
    margin-bottom: 10px;
  }
}
</style>

### Vincent

<div class="member-block" style="display: flex; align-items: flex-start; gap: 20px; flex-wrap: wrap; margin-bottom: 40px;">

  <img src="26-the-deep-learners/static/members/IMG_1052.heic.jpeg"
       alt="Vincent"
       style="width: 150px; height: 150px; object-fit: contain; border-radius: 0%; object-position: center; flex-shrink: 0;">

  <div style="flex: 1; text-align: left;">
    <p style="margin: 0; padding-top: 0.5em;">
      Vincent is a rising senior at Basis Indepent Fremont. 
    </p>
    <ul style="margin: 0; padding-left: 20px;">
      <li>Plays badminton</li>
      <li>Loves to practice math and learn physics</li>
      <li>Very restless sleeper</li>
    </ul>
  </div>

</div>

### William

<div class="member-block" style="display: flex; align-items: flex-start; gap: 20px; flex-wrap: wrap; margin-bottom: 40px;">

  <img src="/26-the-deep-learners/static/members/william liu.heic.jpeg"
       alt="William"
       style="width: 150px; height: 150px; object-fit: contain; border-radius: 0%; object-position: center; flex-shrink: 0;">

  <div style="flex: 1; text-align: left;">
    <p style="margin: 0; padding-top: 0.5em;">
      William is a rising senior at Monta Vista High School. 
    </p>
    <ul style="margin: 0; padding-left: 20px;">
      <li>Plays basketball</li>
      <li>Likes to hang out with friends, eat, and sleep</li>
    </ul>
  </div>

</div>

### William

<div class="member-block" style="display: flex; align-items: flex-start; gap: 20px; flex-wrap: wrap; margin-bottom: 40px;">

  <img src="/26-the-deep-learners/static/members/warrensu.jpg"
       alt="Warren"
       style="width: 150px; height: 150px; object-fit: contain; border-radius: 0%; object-position: center; flex-shrink: 0;">

  <div style="flex: 1; text-align: left;">
    <p style="margin: 0; padding-top: 0.5em;">
      Warren is a rising junior at Lynbrook High School. 
    </p>
    <ul style="margin: 0; padding-left: 20px;">
      <li>Plays basketball</li>
      <li>Likes to watch anime and Joe Bartolozzi</li>
    </ul>
  </div>

</div>


### Rachel Castillo

<div class="member-block" style="display: flex; align-items: flex-start; gap: 20px; flex-wrap: wrap; margin-bottom: 40px;">

  <img src="/26-the-deep-learners/members/Rachel.jpeg"
       alt="Woodstock"
       style="width: 150px; height: 150px; object-fit: contain; border-radius: 25%; object-position: center; flex-shrink: 0;">

  <div style="flex: 1; text-align: left;">
    <p style="margin: 0; padding-top: 0.5em;">
      Rachel is a rising junior at Capo Valley High School in Mission Viejo.
    </p>
    <ul style="margin: 0; padding-left: 20px;">
      <li>Solves Rubkik's competitively</li>
      <li>Likes to cook</li>
      <li>Favorite basketball team is the Warriors</li>
    </ul>
  </div>

</div>
### **Michael Tang**

<div class="member-block" style="display: flex; align-items: flex-start; gap: 20px; flex-wrap: wrap; margin-bottom: 40px;">

  <img src="/26-the-deep-learners/members/woodstock.jpeg"
       alt="michael tang.jpg"
       style="width: 150px; height: 150px; object-fit: contain; border-radius: 25%; object-position: center; flex-shrink: 0;">

  <div style="flex: 1; text-align: left;">
    <p style="margin: 0; padding-top: 0.5em;">
      Michael is a rising junior at Lynbrook High School.
    </p>
    <ul style="margin: 0; padding-left: 20px;">
      <li>Has pet fish</li>
      <li>Likes video games</li>
      <li>Tall</li>
    </ul>
  </div>

</div>
### Woodstock

<div class="member-block" style="display: flex; align-items: flex-start; gap: 20px; flex-wrap: wrap; margin-bottom: 40px;">

  <img src="/26-the-deep-learners/members/woodstock.jpeg"
       alt="Woodstock"
       style="width: 150px; height: 150px; object-fit: contain; border-radius: 25%; object-position: center; flex-shrink: 0;">

  <div style="flex: 1; text-align: left;">
    <p style="margin: 0; padding-top: 0.5em;">
      Woodstock is ...
    </p>
    <ul style="margin: 0; padding-left: 20px;">
      <li>AAA</li>
      <li>BBB</li>
      <li>CCC</li>
    </ul>
  </div>

</div>
### Woodstock

<div class="member-block" style="display: flex; align-items: flex-start; gap: 20px; flex-wrap: wrap; margin-bottom: 40px;">

  <img src="/26-the-deep-learners/members/woodstock.jpeg"
       alt="Woodstock"
       style="width: 150px; height: 150px; object-fit: contain; border-radius: 25%; object-position: center; flex-shrink: 0;">

  <div style="flex: 1; text-align: left;">
    <p style="margin: 0; padding-top: 0.5em;">
      Woodstock is ...
    </p>
    <ul style="margin: 0; padding-left: 20px;">
      <li>AAA</li>
      <li>BBB</li>
      <li>CCC</li>
    </ul>
  </div>

</div>
